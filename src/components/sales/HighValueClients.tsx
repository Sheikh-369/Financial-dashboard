...
const clients = [
  { name: 'Vanguard Tech', type: 'Enterprise Subscription', amount: '$12,400', img: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Vanguard' },
  { name: 'Lumina Group', type: 'Asset Management', amount: '$8,290', img: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Lumina' },
  { name: 'Nexus Retail', type: 'Logistics Core', amount: '$15,100', img: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Nexus' },
];
    ...
    <>
      <div className="bg-white dark:bg-[#132335] p-6 md:p-8 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-sm h-full flex flex-col transition-colors duration-500">
        ...
        <button 
          ...
          className="w-full mt-8 md:mt-10 py-3.5 md:py-4 bg-slate-50 dark:bg-white/5 text-slate-900 dark:text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-100 dark:hover:bg-white/10 transition-all border border-slate-200/50 dark:border-white/5 active:scale-[0.98]"
        >
          View Detailed Pipeline
        </button>
      </div>

      {isPipelineOpen && <PipelineModal onClose={() => setIsPipelineOpen(false)} />}
    </>
  );
}